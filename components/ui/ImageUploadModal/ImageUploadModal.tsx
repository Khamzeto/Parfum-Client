...
const ImageUploadModal: React.FC<UploadPhotoModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUserId(parsedUser.id); // Предполагаем, что ID пользователя хранится в поле id
    }
  }, []);
  ...
  const handleUpload = async () => {
    if (selectedImage && captchaToken && userId) {
      ...
      <Stack>
        ...
        <Group justify="end">
          <Button
            ...
            disabled={!selectedImage || !captchaToken || !userId || uploading}
          >
