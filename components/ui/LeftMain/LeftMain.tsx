...
import { IconBrandTwitter, IconBrandYoutube, IconBrandInstagram, IconCrown, IconCalendar, IconStar, IconPlus, IconMessageCircle, IconHeart, IconShoppingBag, IconMapPin } from '@tabler/icons-react';



export function LeftMain({perfume,posts,stores}) {
  ...
  return (
   <>
    ...
    radius="xl"
    size="lg">
      <IconCrown size={18} color='white' /> {/* Star icon */}
      </ActionIcon>
      
      <Title order={2} style={{ fontSize: '20px', margin: 0 }}>Парфюм дня</Title> {/* Title */}
    
    </Group>

    {/* Perfume Cards */}
    {perfume.map((perfume) => (
      <Card key={perfume._id} className='card-perfume' shadow="2" h='280px'  padding="15.8px " radius="18"  style={{ marginBottom: '20px' }}>
        <Group style={{ display: 'flex', alignItems: 'center', gap: '20px' ,flexDirection: 'column'}}>
          ...
          >
            <Image
              src={perfume.image} // Replace with the actual image
              alt={perfume.name}
              fit="contain"
              width="100%"
              radius='12'
          ...
          <div style={{ flex: 1,textAlign: 'center' }} >
            <Text  size="lg" style={{ color: theme.colors.default}}>{perfume.name}</Text>
            <Text size="sm" style={{ color: theme.colors.gray[6] }}>{perfume.brand}</Text>

            {/* Additional Info */}
            <Group spacing="sm" style={{ marginTop: '10px', alignItems: 'center' }}>
              <IconCalendar size={16} color={theme.colors.blue[6]} />
              <Text size="xs" style={{ color: theme.colors.gray[6] }}>Год выпуска: {perfume.release_year}</Text>
            </Group>
...
{posts.map((post, index) => (
  <div key={post.id}>
    <div style={{ marginBottom: '0px', width: '100%', display: 'flex', alignItems: 'center', gap: '14px'}}>
      <div>
        <Avatar src='https://img.parfumo.de/user_photo/b4/b4_a0ef34c3411634dde703395296642f91c27711a6_200.jpg' alt={post.author} radius="xl" size="lg" />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '0px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginTop: '5px' }}>
            ...
            <Text size="xs" style={{ margin: 0, padding: 0, color: 'gray' }}>122</Text>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginTop: '5px' }}>
            <IconMessageCircle size={16} color='gray' />
            <Text size="xs" style={{ margin: 0, padding: 0, color: 'gray' }}>56</Text>
          </div>
        </div>
      </div>
    </div>
        ...
        <div  key={store.id} style={{marginBottom: '40px'}}>
          <Group className='aromo-shops' align="start" >
            ...
            <Group spacing="xs">
              <Group spacing={5}>
                <IconHeart size={16} color="gray" />
                <Text size="xs" color="dimmed">{store.likes}</Text>
              </Group>
              <Group spacing={5}>
                <IconMessageCircle size={16} color="gray" />
