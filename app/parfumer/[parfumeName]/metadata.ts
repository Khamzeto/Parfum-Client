...
export async function generateMetadata({ params }: { params: { parfumeName: string } }) {
  ...
  try {
    ...
    const title = `${parfumerRu} - Рейтинг созданных ароматов с функцией поиска - Parfumetrika`;
    const description = `Духи от парфюмера ${parfumerName}: его биография, и ключевые достижения. Изучите характеристики каждого парфюма, отзывы пользователей и информацию о брендах, с которыми он сотрудничал. Получите практическое руководство по выбору его ароматов, чтобы найти идеальный вариант.`;
