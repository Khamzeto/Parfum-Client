...
import { IconMars, IconVenus, IconGenderBigender, IconStar, IconCalendar, IconSearch, IconZoomReset, IconGridDots, IconList, IconUser } from '@tabler/icons-react';
...
const PerfumesByParfumer = () => {
  ...
  const fetchPerfumesByParfumer = async (parfumerSlug: string, page: number, sortBy: string, gender: string | null) => {
    setLoading(true);
    try {
...
>
  ...
  <span style={{ color: '#6c757d' }}>
    {parfumerName.split(' ').slice(0, 2).join(' ')}
  </span>
</Breadcrumbs>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px', maxWidth: '1380px', margin: '50px auto' }}>
          <div style={{ flex: 1, maxWidth: '1000px' }}>
            <Title  size='18px' align="left"> {parfumerName}</Title>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', maxWidth: '100%' }}>
              <Button leftSection={<IconZoomReset size={18} />} variant="light" color="gray" onClick={clearFilters}>
                Очистить
              </Button>
            ...
            <div style={{ marginTop: '60px', maxWidth: '1200px' }}>
              {isListView ? (
                <SimpleGrid type="container" cols={{ base: 2, '893px': 2, '720px': 1, '330px': 1, '0px': 1 }} spacing={{ base: '0px', '0px': 'sm' }} style={{ width: '100%' }}>
                  {loading ? (
                    ...
                    perfumes.map((perfume) => (
                      <Link href={`/perfumes/${perfume.perfume_id}`} key={perfume._id} style={{ textDecoration: 'none' }}>
                        <div style={{ display: 'flex', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #eee', gap: '10px' }}>
                          <Image src='https://pimages.parfumo.de/240/196014_img-1075-mellifluence-perfume-ericius_240.webp' alt={perfume.name} width={50} height={50} fit="contain" radius="8" />
                          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                            <Text size="sm" weight={500}>{perfume.name}</Text>
                            <Text size="xs">{perfume.brand}</Text>
                            <Group spacing="xs">
                              <Text size="xs" color="dimmed" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <IconStar size={12} /> {perfume.rating_value} ({perfume.rating_count} отзывов)
                              </Text>
                              <Text size="xs" color="dimmed" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <SimpleGrid type="container" cols={{ base: 4, '893px': 4, '720px': 3, '330px': 2, '0px': 1 }} spacing={{ base: '0px', '0px': 'sm', '300px': 'xs' }} style={{ width: '100%' }}>
                  {loading ? (
                    Array.from({ length: itemsPerPage }).map((_, index) => (
                      <Skeleton key={index} width="100%" height="220px" radius="18" style={{ minWidth: '160px', marginBottom: '20px', maxWidth: '200px' }} />
                    ))
                  ) : (
                    perfumes.map((perfume) => (
                      <Link href={`/perfumes/${perfume.perfume_id}`} key={perfume._id} style={{ textDecoration: 'none' }}>
                        <div style={{ minHeight: '220px', maxWidth: '200px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginBottom: '20px', cursor: 'pointer' }}>
                          <div style={{ height: 100, minWidth: '80%', backgroundColor: '#fff', display: 'flex', justifyContent: 'center', borderRadius: '12px' }}>
                            <Image src='https://pimages.parfumo.de/240/196014_img-1075-mellifluence-perfume-ericius_240.webp' alt={perfume.name} fit="contain" width="60px" />
                          </div>
                          <Text weight={500} align="center" size="xs" style={{ overflow: 'hidden' }}>{perfume.name}{getGenderIcon(perfume.gender)}</Text>
                          {perfume.type && <Badge variant="light" mt="-2px">{perfume.type}</Badge>}
                          <Group spacing="xs">
          ...
          <div className='right-notes' style={{ flex: 1, maxWidth: '250px', textAlign: 'center', marginTop: '180px' }}>
          <div
  style={{
    width: '140px',
    height: '140px',
...
>
  ...
  <Title mb='14' order={4}>{parfumerName}</Title>
