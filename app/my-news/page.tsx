...
export default function Home() {
  ...
  const handleEdit = (articleId) => {
    window.location.href = `/edit-new/${articleId}`;
  };

  const handleDelete = async (articleId) => {
    try {
      await $api.delete(`/news/requests/${articleId}`);
  ...
  return (
    <>
      <head>
        <title>Мои новости | Parfumetrika</title>
        <meta name="description" content="Мои новости,Parfumetrika" />
      </head>
      <Header />
        ...
        <Box style={{ flex: 1 }}>
          <Card padding="20" withBorder radius="14" style={{ display: 'flex' }}>
            <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
              <Text size="lg">Мои Новости</Text>
              <Button onClick={() => router.push('/create-news')} radius="12" maxWidth="200px">
                Создать новость
              </Button>
            </div>
          </Card>
              ...
              {loading
                ...
                : popularArticles.map((article) => (
                    <Card
                      ...
                      shadow="0"
                      withBorder
                      onClick={() => router.push(`/new/${article._id}`)} // Переход на статью при клике
